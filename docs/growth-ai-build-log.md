...
- **Cost cut ~67% ($1.85 → ~$0.61/run)**: search leg → `paul_44~tiktok-search` ($0.0009/result w/ server-side `minPlayCount` floor, ~100% keep rate vs ~40%), baselines → `novi~tiktok-user-api`, both w/ clockworks fallback; verified E2E ($0.44 Apify + $0.02 Gemini + ~$0.15 Claude).
- **Lightreel-demo parity (from the product demo teardown, Jul 3)**: chat-first research — `video_database` tool (searches our free corpus of already-analyzed videos before anything is scraped) + `start_deep_research` tool (chat creates a run; the rail auto-steps it via a `growth:research-started` event); runs reuse the corpus too (seeding queries it first, inspect pulls prior codings + creator baselines from the last 15 runs — a video is never watched twice); memos cite evidence as inline `[@author · views](url)` links and end with follow-up ⚡ chips; plan phase carries DreamMe product context.
