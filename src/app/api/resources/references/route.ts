...
export async function GET(req: Request) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/resource_references?select=*&order=sort_order.desc,created_at.desc`,
      { headers: sbHeaders(), cache: "no-store" },
    );
    if (!res.ok) {
      throw new Error(
        `references read failed: ${res.status} ${await res.text()}`,
      );
    }
...
export async function POST(req: Request) {
  ...
  try {
    // Dedup: if we've already scraped this URL, short-circuit with 409.
    const existing = await fetch(
      `${SUPABASE_URL}/rest/v1/resource_references?select=id&tiktok_url=eq.${encodeURIComponent(tiktokUrl)}&limit=1`,
      { headers: sbHeaders(), cache: "no-store" },
    );
    if (existing.ok) {
      ...
      if (rows.length > 0) {
        return NextResponse.json(
          {
            ok: false,
            error: "This TikTok is already saved in References.",
            id: rows[0].id,
          },
          { status: 409 },
        );
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Only TikTok slideshow posts are supported. This URL looks like a single video.",
        },
        { status: 400 },
      );
    }
    ...
    // `references/{id}/...` before inserting the row.
    const refId = crypto.randomUUID();
    const shortId = refId.slice(0, 8);
    const slides: Array<{ image_url: string; note: string }> = [];
    for (let i = 0; i < slideLinks.length; i++) {
      ...
      const path = `references/${refId}/slide-${String(i).padStart(2, "0")}-${shortId}.jpg`;
      const imageUrl = await fetchToStorage(src, path);
      slides.push({ image_url: imageUrl, note: "" });
    }

    const caption = (item.text ?? "").trim();
    const title =
      caption.split("\n")[0]?.slice(0, 120) || "Untitled reference";
    const authorUsername = item.authorMeta?.name ?? null;

    const insert = await fetch(
      `${SUPABASE_URL}/rest/v1/resource_references`,
      {
        method: "POST",
    ...
    if (!insert.ok) {
      throw new Error(
        `reference insert failed: ${insert.status} ${await insert.text()}`,
      );
    }
    const rows = await insert.json();
    const reference = Array.isArray(rows) ? rows[0] : rows;
